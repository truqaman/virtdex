...
class DexRouter {
  ...
  async getSwapQuote(
    ...
    try {
      ...
      const bestRoute = routes.reduce((best, current) => {
        const bestOutput = ethers.parseUnits(
          best.estimatedOutput,
          outputToken.decimals,
        );
        const currentOutput = ethers.parseUnits(
          current.estimatedOutput,
          outputToken.decimals,
        );
  ...
  private async getVelodromeQuote(
    ...
    try {
      ...
      const isStablePair =
        inputToken.category === "stablecoin" &&
        outputToken.category === "stablecoin";
  ...
  async executeSwap(
    ...
    try {
      ...
      const outputBN = ethers.parseUnits(
        route.estimatedOutput,
        outputToken.decimals,
      );
      const slippage =
        (BigInt(slippageTolerance) * BigInt(10000)) / BigInt(100);
      ...
      console.log(
        `Executing ${route.dex} swap with min output: ${ethers.formatUnits(minOutput, outputToken.decimals)}`,
      );
  ...
  async getOptimalPath(
    ...
    for (const intermediateToken of intermediateTokens) {
      try {
        ...
        const hopQuote = await this.getSwapQuote(
          inputToken,
          intermediateToken,
          inputAmount,
        );
        ...
        const bestOutput = ethers.parseUnits(
          bestRoute.estimatedOutput,
          outputToken.decimals,
        );
        const hopOutput = ethers.parseUnits(
          secondHopQuote.bestRoute.estimatedOutput,
          outputToken.decimals,
        );
        ...
        console.error(
          `Error checking hop through ${intermediateToken.symbol}:`,
          error,
        );
      }
    }
